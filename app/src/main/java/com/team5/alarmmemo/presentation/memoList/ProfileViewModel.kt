...
@HiltViewModel
class ProfileViewModel @Inject constructor(private val remoteUserDataRepository: RemoteUserDataRepository, @RemoteRepository private val remoteMemoDataRepository: MemoDataRepository):ViewModel() {
    ...
    fun modifyUserData(user: User){
        remoteUserDataRepository.addOrModifyUserData(user){ bool, e ->
            if(bool){
    ...
    fun deleteId(id:String){
        remoteMemoDataRepository.removeIdContent {
            if(it){
                remoteUserDataRepository.deleteUserData(id){ bool, e ->
                    if(bool){
